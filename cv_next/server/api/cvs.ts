...
export async function getCvById(cvId: string): Promise<CvModel | null> {
  try {
    ...
    if (error) {
      logger.error("cvs::getCvById", error);
      return null;
    }
...
export async function getCvsByUserId(
  ...
  try {
    ...
    if (error) {
      logger.error("getCvsByUserId", error);
      return null;
    }
...
export async function getAllCvsByCategory(
  ...
  try {
    ...
    if (error) {
      logger.error("getAllCvsByCategory", error);
      return null;
    }
...
async function _getAllCvsByCategories(
  ...
  try {
    ...
    if (error) {
      logger.error("getAllCvsByCategories", error);
      return error;
    }
...
export async function getPaginatedCvs(
  ...
  try {
    ...
    if (error) {
      logger.error("getPaginatedCvs", error);
      return null;
    }
...
export async function updateCV(cv: CvModel): Promise<PostgrestError | null> {
  try {
    const { error } = await SupabaseHelper.getSupabaseInstance()
      ...
      .eq(CvKeys.id, cv.id);
    if (error) {
      logger.error("cvs::updateCV", error);
      return error;
    }
    return null;
  } catch (error) {
