...
export const FilterPanel = ({defaultFilters, onChange, cvs}: {
    ...
    return (
        <div className="mx-10 my-[10px] flex flex-row justify-between items-center gap-2">
            <SearchBox placeHolder=" input text to search" value={searchValue} onChange={(value: string) => {setSearchValue(value)}}></SearchBox>
            <DropdownInput 
                placeHolder="all" 
                valueIds={Object.keys(Categories.category).map(key=> parseInt(key)).filter(key => !isNaN(key))} 
                text="catagory" 
                valueId={categoryId} 
