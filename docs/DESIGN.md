...
As this table is auto generated by Supabase, we only use several fields from it.
...
| full_name              | text                     |
| avatar_url             | text                     |
| work_status            | work status enum         |
| work_status_categories | categories enum array    |
...
| id            | uuid                     |
| user_id       | uuid                     |
| cv_categories | CV categories enum array |
...
| deleted           | boolean                  |
| document_id       | uuid                     |
| upvotes           | user ids array           |
| last_update       | timestamp with time zone |

### Admins/Whitelisted Views

Based on the data from the perms table

- Users with Admin are in both admins and whitelisted
...
- Each file there is the first page of the CV with the name being the docs ID.
